 ...
 */
async function generateSpecification(input) {
    try {
        ...
        const requirementsData = JSON.parse((await fs.readFile(projectFilePath)).toString());
        ...
        const specification = generateSpecificationDocument(requirementsData, input.format, includeSections);
        return {
            success: true,
            specification,
            format: input.format,
 ...
 */
function generateSpecificationDocument(requirements, format, includeSections) {
    if (format === 'json') {
 ...
 */
function generateJsonSpecification(requirements, includeSections) {
    const specification = {
    ...
    if (includeSections.includes('functionalRequirements')) {
        specification.functionalRequirements = requirements.requirementCategories.functional.map((req, index) => ({
            id: `F${index + 1}`,
    ...
    if (includeSections.includes('nonFunctionalRequirements')) {
        specification.nonFunctionalRequirements = requirements.requirementCategories.nonFunctional.map((req, index) => ({
            id: `NF${index + 1}`,
            description: req.answer,
            source: req.source,
            relatedQuestion: req.question
    ...
    if (includeSections.includes('constraints')) {
        specification.constraints = requirements.requirementCategories.constraints.map((req, index) => ({
            id: `C${index + 1}`,
            description: req.answer,
            source: req.source,
            relatedQuestion: req.question
    ...
    if (includeSections.includes('assumptions')) {
        specification.assumptions = requirements.requirementCategories.assumptions.map((req, index) => ({
            id: `A${index + 1}`,
            description: req.answer,
            source: req.source,
            relatedQuestion: req.question
 ...
 */
function generateMarkdownSpecification(requirements, includeSections) {
    let markdown = '';
    // Title
    ...
    if (includeSections.includes('stakeholders') && requirements.basicInfo.stakeholders?.length > 0) {
        ...
        requirements.basicInfo.stakeholders.forEach((stakeholder, index) => {
    ...
    if (includeSections.includes('functionalRequirements')) {
        ...
        if (requirements.requirementCategories.functional.length === 0) {
            ...
            requirements.requirementCategories.functional.forEach((req, index) => {
    ...
    if (includeSections.includes('nonFunctionalRequirements')) {
        ...
        if (requirements.requirementCategories.nonFunctional.length === 0) {
            ...
            requirements.requirementCategories.nonFunctional.forEach((req, index) => {
    ...
    if (includeSections.includes('constraints')) {
        ...
        if (requirements.requirementCategories.constraints.length === 0) {
            ...
            requirements.requirementCategories.constraints.forEach((req, index) => {
    ...
    if (includeSections.includes('assumptions')) {
        ...
        if (requirements.requirementCategories.assumptions.length === 0) {
            ...
            requirements.requirementCategories.assumptions.forEach((req, index) => {
...
body('projectId').isString().notEmpty().withMessage('Project ID is required'), body('format').optional().isIn(['markdown', 'json']), body('includeSections').optional().isArray(), async (req, res) => {
...
//# sourceMappingURL=generate-specification-tool.js.map