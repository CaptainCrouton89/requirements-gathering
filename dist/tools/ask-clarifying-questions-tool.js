 ...
 */
function generateClarifyingQuestions(requirements) {
 ...
 */
async function askClarifyingQuestions(input) {
    try {
 ...
 */
async function answerClarifyingQuestions(input) {
    try {
        const projectFilePath = path.join(dataDir, `${input.projectId}.json`);
        // Check if the project exists
        try {
        ...
        requirementsData.clarifyingQuestions =
            requirementsData.clarifyingQuestions.filter((q) => !answeredQuestionKeys.includes(q));
...
body('projectId').isString().notEmpty().withMessage('Project ID is required'), body('generateNew').optional().isBoolean(), async (req, res) => {
...
router.post('/answer', 
// Validation middleware
body('projectId').isString().notEmpty().withMessage('Project ID is required'), body('answers').isObject().notEmpty().withMessage('Answers object is required'), async (req, res) => {
    // Check for validation errors
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
...
//# sourceMappingURL=ask-clarifying-questions-tool.js.map