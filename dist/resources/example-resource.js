...
// GET item by ID
router.get('/:id', param('id').isString().notEmpty().withMessage('Valid ID is required'), (req, res) => {
...
// CREATE item
router.post('/', body('name').isString().notEmpty().withMessage('Name is required'), body('description').optional().isString().withMessage('Description must be a string'), (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
// UPDATE item
router.put('/:id', param('id').isString().notEmpty().withMessage('Valid ID is required'), body('name').optional().isString().notEmpty().withMessage('Name cannot be empty'), body('description').optional().isString().withMessage('Description must be a string'), (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
...
// DELETE item
router.delete('/:id', param('id').isString().notEmpty().withMessage('Valid ID is required'), (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
