...
type StartRequirementsInput = z.infer<typeof startRequirementsSchema>;

/**
 * Start requirements tool handler
 */
async function startRequirementsTool(input: StartRequirementsInput): Promise<{ 
  success: boolean, 
  projectId: string,
  message: string,
  nextSteps: string[]
...
router.post('/', 
  ...
  body('projectName').isString().notEmpty().withMessage('Project name is required'),
  body('projectDescription').isString().notEmpty().withMessage('Project description is required'),
  body('projectType').isIn(['web', 'mobile', 'desktop', 'api', 'other']).withMessage('Valid project type is required'),
  body('projectTypeDetails').optional().isString(),
  body('targetCompletion').optional().isString(),
